...
public interface UserMapper {
    ...
    @Select("select b.name,b.headportrait,b.nikename,b.create_time,a.status,a.type,a.phonenumber from gy_user_security a,gy_user b " +
            "where a.user_code=b.code and b.name like CONCAT('%',#{text},'%') or a.user_code=b.code and b.nikename like CONCAT('%',#{text},'%')  or a.user_code=b.code and a.phonenumber like CONCAT('%',#{text},'%') " +
            "order by b.create_time desc LIMIT #{page},#{size}")
    List<JSONObject> getAllByPage(int page, int size,String text);
