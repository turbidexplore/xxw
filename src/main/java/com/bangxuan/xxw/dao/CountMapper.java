...
public interface CountMapper {
    ...
    @Select("select count(*) from user_logs where date(create_time)=date(#{date}) ")
    Integer fwlCountByDate(@Param("date")String date);

    @Select("select * from user_logs where userinfo ${sql} like '%-%' and date(create_time)=date(#{date}) group by userinfo")
    List<JSONObject> todayuserCountByDate(@Param("sql")String sql,@Param("date")String date);

    @Select("select date(create_time) from user_logs\n" +
            "where userinfo like '%-%'  group by date(create_time) order by create_time desc")
    List<String> dates();

    @Insert("insert into count_logs(a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p,q,r,s,t,u,v,w,x,y,z,today)" +
            " values(#{a},#{b},#{c},#{d},#{e},#{f},#{g},#{h},#{i},#{j},#{k},#{l},#{m},#{n},#{o},#{p},#{q},#{r},#{s},#{t},#{u},#{v},#{w},#{x},#{y},#{z},date_format(now(),'%Y-%m-%d'))")
    int addLogs(JSONObject data);

    @Select("select * from count_logs order by today desc")
    List<JSONObject> countlogs();

    @Insert("insert into follow_class(userinfo,classid,companyid,ip,create_time) values(#{userinfo},#{classid},#{companyid},#{ip},date_format(now(),'%Y-%m-%d'))")
    Integer addFollowClass(JSONObject jsonObject);

    @Select("select c.company_name,b.class_name,e.nikename,a.userinfo,a.ip,a.create_time\n" +
            "from follow_class a\n" +
