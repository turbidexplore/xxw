...
//                productClassService.updateUser(principal.getName(), data.getString("id"));
    ...
    @PutMapping("/saveProductSkuInfos")
    public Mono<Message> saveProductSkuInfos(@RequestBody JSONObject productskuinfos, @RequestParam("classId") String classId) {
            JSONArray jsonArray = productskuinfos.getJSONArray("productskuinfos");
            // 保存【下一步】表达式到gy_class_express表中
            Express express = expressService.getByClassId(classId);
            express.setBisexpressjson(jsonArray.toJSONString());
            expressService.update(express);
            skuThread.saveProductSkuInfos(classId,jsonArray);
...
//        {"skuname":["",""],"value":"","type":11},saveSkuInfos
